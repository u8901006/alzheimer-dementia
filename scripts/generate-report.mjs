...
const TOPIC_TAGS = [
  "阿茲海默症", "失智症", "類澱粉蛋白", "tau蛋白", "生物標記",
  "神經影像", "臨床試驗", "認知功能", "照護者", "社會決定因素",
  "營養與飲食", "運動與身體活動", "睡眠", "MCI輕度認知障礙",
  "藥物治療", "神經發炎", "遺傳學", "病理機制", "預防",
  "生活品質", "神經心理學", "長期照護", "健康政策", "數位生物標記",
  "類澱粉蛋白清除療法", "血管性失智症", "路易體失智症", "額顳葉失智症",
];
...
function buildPrompt(papersData) {
  ...
  return `以下是 ${dateStr} 從 PubMed 抓取的最新阿茲海默症與失智症相關文獻（共 ${paperCount} 篇）。
...
每篇 paper 的 tags 請從以下選擇：${TOPIC_TAGS.join("、")}。
    ...
    if (jsonMatch) {
      try {
        ...
        if (braceStart >= 0 && braceEnd > braceStart) {
          let sub = cleaned.slice(braceStart, braceEnd + 1);
    ...
    return `
        <div class="news-card featured">
          <div class="card-header">
            ...
            <span class="emoji-icon">${p.emoji || "\u{1F4C4}"}</span>
            <span class="${ucls}">${esc(util)}實用性</span>
          </div>
          ...
          <div class="card-footer">
            ${tags}
            <a href="${escAttr(p.url || "#")}" target="_blank">閱讀原文 &#8594;</a>
          </div>
        </div>`;
  }).join("");
    ...
    return `
        <div class="news-card">
          <div class="card-header-row">
            ...
            <span class="${ucls} utility-sm">${esc(util)}</span>
          </div>
          <h3>${esc(p.title_zh || p.title_en || "")}</h3>
          ...
          <div class="card-footer">
            ${tags}
            <a href="${escAttr(p.url || "#")}" target="_blank">PubMed &#8594;</a>
          </div>
        </div>`;
  }).join("");
...
<title>Alzheimer Dementia Daily &middot; 阿茲海默症文獻日報 &middot; ${dateDisplay}</title>
<meta name="description" content="${dateDisplay} 阿茲海默症與失智症文獻日報，由 AI 自動彙整 PubMed 最新論文"/>
<style>
...
<div class="container">
  <header>
    ...
    <div class="header-text">
      <h1>Alzheimer Dementia Daily &middot; 阿茲海默症文獻日報</h1>
  ...
  <footer>
    ...
    <span><a href="https://github.com/u8901006/alzheimer-dementia">GitHub</a></span>
  </footer>
</div>
</body>
</html>`;
...
async function main() {
  ...
  if (!papersData?.papers?.length) {
    ...
    const emptyAnalysis = {
      ...
      market_summary: "今日 PubMed 暫無新的阿茲海默症與失智症文獻更新。請明天再查看。",
  ...
  const analysis = await analyzeWithFallback(apiKey, prompt);
